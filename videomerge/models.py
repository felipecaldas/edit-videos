...
class BrandKitRef(BaseModel):
    """Reference to the client's brand kit resolved at render time.

    Attached to the Brief by the n8n webhook from the request's brand_kit_id;
    never generated by the LLM. Downstream consumers (brand_prompt.py,
    FalClient, Remotion compositor) read this to apply the correct Recraft
    style_id, negative prompts, and brand assets without any LLM-invented text.
    """

    client_id: str = Field(
        ...,
        description="Supabase clients.id whose brand_profiles row carries the kit.",
        examples=["client-42"],
    )
    brand_kit_version: Optional[str] = Field(
        None,
        description="Opaque version token (e.g. a timestamp or commit SHA) so consumers can detect stale kits.",
        examples=["2026-04-26T00:00:00Z"],
    )


class VisualDirection(BaseModel):
    """Shared cinematic direction cues applied across all platform briefs.

    Describes emotional and photographic qualities only — mood, shot style,
    colour grading. Must never contain brand names, product names, logos,
    typography, UI elements, or any company-specific visual identity. Brand
    assets are applied by the rendering pipeline via BrandKitRef.
    """
    ...
    color_feel: Optional[str] = Field(
        None,
        description="Lighting and colour mood (e.g. 'warm golden hour', 'cool desaturated blue'). Never a brand palette.",
        examples=["warm golden hour"],
    )
    shot_style: Optional[str] = Field(
        None,
        description="Camera technique (e.g. 'handheld documentary', 'locked-off wide'). Never a brand format.",
...
class Brief(BaseModel):
    ...
    visual_direction: Optional[VisualDirection] = Field(
        None,
        description="Shared cinematic direction cues (mood, shot style, colour feel). Never contains brand identity.",
    )
    ...
    brand_kit_ref: Optional[BrandKitRef] = Field(
        None,
        description=(
            "Reference to the client's brand kit. Populated by the n8n webhook from the request's brand_kit_id; "
            "never generated by the LLM. Used by downstream pipeline stages to resolve Recraft style_id, "
            "negative prompts, and brand assets without any model-invented text."
        ),
    )
