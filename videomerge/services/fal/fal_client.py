...
class FalClient:
    ...
    async def upload_file(self, file_path: str) -> str:
        ...
        logger.info("[fal] Uploaded %s → %s", file_path, url)
        return url

    _ECHOMIMIC_MODEL = "fal-ai/echomimic-v3"
    _ECHOMIMIC_DEFAULT_PROMPT = (
        "A person is in a relaxed pose. As the video progresses, the character speaks while "
        "arm and body movements are minimal and consistent with a natural speaking posture. "
        "Hand movements remain minimal. Don't blink too often. Preserve background integrity "
    ...
    async def submit_echomimic(
        ...
        """Submit a talking-head job to fal-ai/echomimic-v3.

        Args:
            image_url: Portrait image URL (already-generated by Fal image gen).
            audio_url: URL of the audio segment for this scene.
            prompt: Motion prompt; defaults to _ECHOMIMIC_DEFAULT_PROMPT.

        Returns:
            Request ID for polling.
        """
        effective_prompt = prompt or self._ECHOMIMIC_DEFAULT_PROMPT
        arguments = {
            "image_url": image_url,
            "audio_url": audio_url,
        ...
        logger.info("[fal] Submitting echomimic-v3 job: image=%s, audio=%s", image_url, audio_url)

        handler = await fal_client.submit_async(self._ECHOMIMIC_MODEL, arguments=arguments)
        request_id = handler.request_id

        logger.info("[fal] echomimic-v3 job submitted: request_id=%s", request_id)
        return request_id
    ...
    async def poll_echomimic(
        ...
        """Poll echomimic-v3 until complete, download the video, return local path.

        Args:
            request_id: Request ID from submit_echomimic.
            output_dir: Directory to download the finished video into.
        ...
        Raises:
            TimeoutError: If the job doesn't complete within timeout_s.
            RuntimeError: If the job fails or returns no video.
        """
        start_time = time.time()
        ...
        while time.time() - start_time < timeout_s:
            try:
                status = await fal_client.status_async(
                    self._ECHOMIMIC_MODEL, request_id, with_logs=False
                )

                if isinstance(status, FalCompleted):
                    if status.error:
                        raise RuntimeError(f"echomimic-v3 job failed: {status.error}")

                    result = await fal_client.result_async(self._ECHOMIMIC_MODEL, request_id)
                    video = result.get("video", {})
                    video_url = video.get("url") if isinstance(video, dict) else None

                    if not video_url:
                        raise RuntimeError(f"echomimic-v3 returned no video URL: {result}")

                    paths = await self.download_outputs([video_url], output_dir, index=0)
                    local_path = str(paths[0])
                    logger.info("[fal] echomimic-v3 video saved: %s", local_path)
                    return local_path
                    ...
                    logger.warning(
                        "[fal] echomimic-v3 unknown status '%s', retrying: request_id=%s",
                        type(status).__name__, request_id,
                    )
                    await asyncio.sleep(poll_interval_s)
                ...
                if 400 <= e.status_code < 500:
                    raise RuntimeError(f"echomimic-v3 API error ({e.status_code}): {e}")
                logger.warning("[fal] echomimic-v3 HTTP %d, retrying: %s", e.status_code, e)
                await asyncio.sleep(poll_interval_s)

            except Exception as e:
                if isinstance(e, (RuntimeError, TimeoutError)):
                    raise
                logger.warning("[fal] echomimic-v3 poll error: %s, retrying", e)
                await asyncio.sleep(poll_interval_s)

        raise TimeoutError(
            f"Timed out waiting for echomimic-v3: request_id={request_id}, timeout={timeout_s}s"
        )
